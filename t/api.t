...
subtest 'callback() tests' => sub {
    ...
    my ( $patron, $amount, $amount_cents, $sum_cents, $debit_line, $template ) = _create_test_data();
...
subtest 'create_transaction() tests' => sub {
    ...
    my ( $patron, $amount, $amount_cents, $sum_cents, $debit_line, $template ) = _create_test_data(4);
...
HERE
    ...
    $mocked_lwp->mock(
        'get' => sub {
            return HTTP::Response->new(
                200, 'OK', HTTP::Headers->new, <<HERE
{"paymentOrder":{"orderItems": {"orderItemList":[]}, "paid": { "id": "test" }, "status": "paid"}, "operations": [], "paid": { "amount": 1 }}
HERE
            );
        }
    );
    ...
    $mocked_lwp->mock(
        'get' => sub {
            return HTTP::Response->new(
                400, 'OK', HTTP::Headers->new, <<HERE
{"paymentOrder":{"orderItems": {"orderItemList":[]}, "paid": { "id": "test" }, "status": "paid"}, "operations": [], "paid": { "amount": 50000 }}
HERE
            );
        }
    );
