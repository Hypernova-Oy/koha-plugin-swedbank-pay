...
sub opac_online_payment_begin {
    ...
    if ($error) {
        ...
        C4::Log::logaction(
            'FINES', 'UPDATE', $borrowernumber,
            "Swedbank payment started. Order ID: $order_id, Payment ID: $paymentorder_id"
...
sub create_transaction {
    ...
    my $decimals       = $self->get_currency_precision;
    ...
    for my $accountline ( $accountlines->all ) {
        ...
        if ( $decimals > 0 ) {
            $acc_amount = int( $acc_amount * 10**$decimals );
        }
        push @order_items, {
    ...
    try {
        ...
        if ( blessed $_ ) {
            if ( $_->isa('Koha::Plugin::Com::imCode::SwedbankPay::Exceptions::APIError') ) {
                ...
                C4::Log::logaction(
                    'FINES', 'UPDATE', $borrowernumber,
                    "Swedbank payment API error. "
                        . ref($_) . ': '
                        . $_->error
                        . ', dump: '
                        . Data::Dumper::Dumper( $_->field_hash )
                ) if C4::Context->preference("FinesLog");
            } else {
                $self->logger->error( ref($_) . ': ' . $_ );
                C4::Log::logaction(
                    'FINES', 'UPDATE', $borrowernumber,
                    "Swedbank payment error. " . ref($_) . ': ' . $_
                ) if C4::Context->preference("FinesLog");
            }
        } else {
            $self->logger->error("Swedbank Payments Plugin: $_");
            C4::Log::logaction( 'FINES', 'UPDATE', $borrowernumber, "Swedbank payment error. $_" )
                if C4::Context->preference("FinesLog");
        }
    };
    ...
    try {
        ...
        if ( blessed $_ ) {
                ...
                || $_->isa('Koha::Plugin::Com::imCode::SwedbankPay::Exceptions::APIError') )
            {
                ...
                C4::Log::logaction(
                    'FINES', 'UPDATE', $borrowernumber,
                    "Swedbank payment processing error. "
                        . ref($_) . ': '
                        . $_->error
                        . ', dump: '
                        . Data::Dumper::Dumper( $_->field_hash )
                ...
                C4::Log::logaction(
                    'FINES', 'UPDATE', $borrowernumber,
                    "Swedbank payment processing error. " . ref($_) . ': ' . $_
                ) if C4::Context->preference("FinesLog");
            }
        } else {
            $template->param( message => 'unknown_error', order_id => $order_id );
            $self->logger->error("Swedbank Payments Plugin: $_");
            C4::Log::logaction( 'FINES', 'UPDATE', $borrowernumber, "Swedbank payment processing error. $_" )
                if C4::Context->preference("FinesLog");
        }
    };
    ...
    try {
        $schema->txn_do(
            sub {
                my $dbh = C4::Context->dbh;
                my $sth = $dbh->prepare(
                    "SELECT transaction_id, borrowernumber, swedbank_paymentorder_id, payee_reference, amount FROM $table WHERE order_id = ?"
                );
                $sth->execute($order_id);
                my ( $transaction_id, $patron_id, $paymentorder_id, $payee_reference, $amount ) =
                    $sth->fetchrow_array();
                ...
                my $host        = $self->get_host;
                my $url         = "$host" . "$paymentorder_id" . '?$expand=orderItems,paid';
                my @req_headers = $self->get_headers;
                ...
                if ( $res->is_success && $res->code == 200 ) {
                            ...
                            or !$res_json->{paymentOrder}->{orderItems}->{orderItemList} )
                        {
                            ...
                            if ( $cap_res->is_success && $cap_res->code == 200 ) {
                                ...
                                if ( $res->is_success && $res->code == 200 ) {
                                    ...
                                    C4::Log::logaction(
                                        'FINES', 'UPDATE', $borrowernumber,
                                        "Swedbank payment captured. Order ID: $order_id, Paymentorder ID: $paymentorder_id"
                                    ) if C4::Context->preference("FinesLog");
                    ...
                    my $paid_url = "$host" . "$paymentorder_id" . '?$expand=orderItems,paid';

                    if ($paid_url) {
                        $res = $ua->get( $paid_url, @req_headers );
                        if ( $res->is_success && $res->code == 200 ) {
                            ...
                            if ( exists $res_json->{paymentOrder}->{paid}->{amount}
                                && $res_json->{paymentOrder}->{paid}->{amount} == $amount )
                            {
                                $paid = 1;
                                ...
                                C4::Log::logaction(
                                    'FINES', 'UPDATE', $borrowernumber,
                                    "Swedbank payment finished and paid. Order ID: $order_id, Payment ID: $paymentorder_id"
                                ) if C4::Context->preference("FinesLog");
                                ...
                                my $status      = $res_json->{paymentOrder}->{status};
                                my $paid_amount = $res_json->{paymentOrder}->{paid}->{amount} || '';
                                $amount ||= 0;
                                if ( lc( $res_json->{paymentOrder}->{status} ) ne 'paid' ) {
                                    Koha::Plugin::Com::imCode::SwedbankPay::Exceptions::APIError::PaymentError->throw(
                                        error_code => 'cancelled_payment',
                                        error      =>
                                            "Payment status '$status' != 'Paid'. Paid amount '$paid_amount', total amount: '$amount'.",
                                        content  => $res->decoded_content,
                                        order_id => $order_id,
                                ...
                                Koha::Plugin::Com::imCode::SwedbankPay::Exceptions::APIError::PaymentError->throw(
                                    ...
                                    order_id   => $order_id,
                                    property   => 'paid'
                                ) unless exists $res_json->{paymentOrder}->{paid};
                                Koha::Plugin::Com::imCode::SwedbankPay::Exceptions::APIError::PaymentError->throw(
                                    error_code => 'PAYMENT_PAID_FAILED_PAID_AMOUNT_NOT_EQUAL_TO_TOTAL',
                                    error      => "Paid amount '$paid_amount' != '$amount'",
                                    content    => $res->decoded_content,
                                    order_id   => $order_id,
                                    status     => $res_json->{paymentOrder}->{status},
                                );
...
sub configure {
    ...
    unless ( $cgi->param('save') ) {
        ...
        my $currencyPrecision = $self->retrieve_data('SwedbankCurrencyPrecision') || 2;
        $template->param(
            ...
            SwedbankCurrencyPrecision   => $currencyPrecision,
        ...
        my $currencyPrecision = $cgi->param('SwedbankCurrencyPrecision') || 2;
        $self->store_data(
            {
...
sub get_currency_precision {
    my ($self) = @_;
    my $stored_precision = $self->retrieve_data('SwedbankCurrencyPrecision');
    if ($stored_precision) {
